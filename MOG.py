...
while (1):  # 摄像头正常，进入循环体，读取摄像头每一帧图像
    ...
    cv2.imshow("fgmask", fgmask)  # 显示运动前景图像

    image = cv2.cvtColor(fgmask, cv2.COLOR_GRAY2BGR)
    # cv2.imshow("color", image)
    print(fgmask.shape)
    print(frame.shape)

    out_detect_pictures = cv2.imwrite("pictures/{}.jpg".format(i), fgmask)
